...
class WallAroundTest(unittest.TestCase):
  ...
  def test_io(self):
    left, right = self.set_and_get(51,0,0,0) #curve to right
    self.assertTrue(left > right,"don't curve to right")

    left, right = self.set_and_get(0,0,0,51) #curve to right
    self.assertTrue(left > right,"don't curve to right")

    left, right = self.set_and_get(0,0,51,0) #curve to left
